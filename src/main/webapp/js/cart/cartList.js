	...
	modCarts.forEach(modCart => {
		modCart.addEventListener("click", function (e) {
			...
			if (confirm("수정하시겠습니까?")) {
				...
				let proNo = modCart.dataset.product_no
				let proname = modCart.dataset.product_name
				let proprice = modCart.dataset.price
				let proimg = modCart.dataset.img
				console.log(cartNo, curQuantity, modquantity, memberNo1, product_No, product_name, price, img);
				if (curQuantity != modquantity) {
					fetch("modifyCart.do?cno=" + cartNo + "&quan=" + modquantity + "&mno=" + memberNo1 + "&bno=" + proNo + "&productName=" + proname + "productPrice=" + proprice + "&productImg=" + proimg)
						.then(result => result.json())
						.then(result => {
	...
	orderBtn.addEventListener("click", function (e) {
		...
		checks.forEach(check => {
			...
			if (check.checked == true) {
				...
				let proNo = check.dataset.bookno;
				let proImg = check.dataset.img;
				let proName = check.dataset.name;
				let proPirce = check.dataset.price;
				let quantity = check.dataset.quantity;
				let order = { "cart_No": cartNo, "product_No": proNo, "product_Img": proImg, "product_Name": proName, "product_Pirce": proPirce, "quantity": quantity }
				orders.push(order);
				console.log(order);
